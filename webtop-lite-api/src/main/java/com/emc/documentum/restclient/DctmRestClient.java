...
import com.emc.documentum.restclient.model.Properties;
import com.emc.documentum.restclient.model.UserModel;

@Component("DctmRestClient")
@PropertySource("classpath:application.properties")
@Deprecated
public class DctmRestClient {

	private static final Log LOGGER = LogFactory.getLog(DctmRestClient.class);
	@Autowired
	...
	public UserModel getUserInfo(String username, String password) {
		...
		ResponseEntity<UserModel> response = restTemplate.exchange("current-user-url", HttpMethod.GET,
	...
	public JsonObject deleteObject(String objectId , boolean deleteChildrenOrNot) throws CanNotDeleteFolderException {
		try {
			RestTemplate restTemplate = new RestTemplate();
			String objectUri = "object-url" + "/" + objectId;
	...
	public JsonObject getCabinet(String cabinetName) throws CabinetNotFoundException {
		...
		RestTemplate restTemplate = new RestTemplate();
		String URI = "cabinet-url" + "?filter=starts-with(object_name,'" + cabinetName + "')";
	...
	public JsonObject getFolderByPath(String queryFolderPath) throws FolderNotFoundException {
		...
		String URI = String.format(
				"dql-url" + "select *,r_folder_path from dm_folder where any r_folder_path = '%s'",
				queryFolderPath);
	...
	public List<JsonEntry> getAllCabinets(int pageNumber, int pageSize) {
		RestTemplate restTemplate = new RestTemplate();
		String URI = "dql-url" + "select * from dm_cabinet";
	...
	public List<JsonEntry> getChildren(String folderId, int pageNumber, int pageSize) {
		RestTemplate restTemplate = new RestTemplate();
		String URI = "dql-url" + "select *,r_lock_owner from dm_folder where  FOLDER(ID('" + folderId
	...
	private byte[] getContentBase64Content(JsonObject content) {
		try {
			...
			if (resource.getBody() != null) {
				...
				LOGGER.info(content.getName() + " has empty content");
			}
		} catch (IOException e) {
			LOGGER.error("IO Exception while reading dm_document input stream", e);
		}
	...
	public List<JsonEntry> getDocumentByName(String name) throws DocumentNotFoundException {
		RestTemplate restTemplate = new RestTemplate();
		String URI = String.format("dql-url" + "select * from dm_sysobject where object_name like '%s'",
				"%" + name + "%");
		ResponseEntity<JsonFeed> response = restTemplate.exchange(URI, HttpMethod.GET,
				new HttpEntity<Object>(createHeaders(data.username, data.password)), JsonFeed.class);
	...
	public JsonObject checkinDocument(String documentId, byte[] content) throws DocumentCheckinException {
		...
		parts.add("data", Base64.decodeBase64(content));
	...
	public List<com.emc.documentum.restclient.model.JsonEntry> getPaginatedResult(String folderId, int startIndex, int pageSize) {
//		// TODO check count first and check it against page size
//		String query = String.format(
//				"select * from dm_folder where FOLDERS(id('%s')) ENABLE(RETURN_RANGE 1 1 'r_creation_date ASC' ) ",
//				folderId);
...
//		return folders;
	...
	private ResponseEntity<JsonFeed> executeDQL(String query) {
		RestTemplate restTemplate = new RestTemplate();
		// LOGGER.LOGGER(level, msg, thrown);
		String URI = "dql-url" + query;
		ResponseEntity<JsonFeed> response = restTemplate.exchange(URI, HttpMethod.GET,
				new HttpEntity<Object>(createHeaders(data.username, data.password)), JsonFeed.class);
		return response;
	}
